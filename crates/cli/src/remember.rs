...
// - this default in invoked whenever triodion is run without specifying datatypes
...
pub(crate) struct RememberedCommand {
    pub(crate) triodion_version: String,
    pub(crate) command: Vec<String>,
    pub(crate) args: Args,
}

pub(crate) fn save_remembered_command(
    triodion_dir: PathBuf,
    args: &Args,
) -> Result<(), ParseError> {
    let triodion_version = triodion_core::TRIODION_VERSION.to_string();
    let args = Args { remember: false, ..args.clone() };
    let command = std::env::args().filter(|w| w != "--remember").collect::<Vec<_>>();

    let remembered = RememberedCommand { triodion_version, command, args };

    let json = serde_json::to_string(&remembered).map_err(|_| {
        ParseError::ParseError("could not serialize remembered command".to_string())
    })?;
    let path = get_remembered_command_path(triodion_dir)?;
...
pub(crate) fn load_remembered_command(
    triodion_dir: PathBuf,
) -> Result<RememberedCommand, ParseError> {
    let path = get_remembered_command_path(triodion_dir)?;
...
pub(crate) fn get_remembered_command_path(triodion_dir: PathBuf) -> Result<PathBuf, ParseError> {
    Ok(triodion_dir.join(REMEMBER_FILENAME))
}
